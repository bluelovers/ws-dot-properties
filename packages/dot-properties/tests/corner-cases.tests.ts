...
describe('lines', () =>
{
	...
	test('write lines', () =>
	{
		...
		expect(lines).not.toMatchObject(exp)
		expect(lines).toMatchSnapshot()
		const fix = str.replace('# The exclamation mark', '! The exclamation mark')
		expect(lines = parseLines(fix)).toMatchObject(exp)
		expect(lines).toMatchSnapshot()
	})
})
...
deserunt mollit anim id est laborum.`
		...
		const res0 = stringify([lorem])
		expect(res0).toBe(lorem.replace(/\r\n/gm, '\n# ').trim())

		const res1 = stringify([['key', lorem]])
		expect(res1).toBe(
			'key = ' + lorem.replace(/\r\n/g, '\\r\\n\\\n    '),
		)
